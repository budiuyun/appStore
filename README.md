...
为了让用户更容易发现和使用您的应用，我们引入了自定义分类标签系统。当您提交Helm应用时，请在Chart.yaml中添加`budiu/app-category-zh`标签。

### 如何添加分类标签

在`Chart.yaml`文件中，添加以下内容：

```yaml
annotations:
  budiu/app-category-zh: "数据库"  # 替换为适合您应用的分类
```

### 支持的分类

我们建议使用以下标准分类，以确保在应用商店中有一致的展示：
...
我们的自动化工作流会检测所有应用的分类标签，并生成一个`category-index.json`文件，该文件会被前端用来展示正确的分类信息。

如果您需要自定义分类，请在Issue中提出建议。
